...
%
\sloppy{
\title{Humanoid Robots and Spoken Dialog Systems for Brief Health Interventions}
...
%We developed a virtual counseling system which can deliver brief alcohol health interventions via a 3D anthropomorphic speech-enabled interface. Here, we present an adaptation of the system, combining it with a fully autonomous humanoid robot (NAO). 
...
interaction. 
%We compared the unoptimized system with the optimized system in terms of objective measures (e.g. task completion) and subjective measures (e.g. ease of use, future intention to use the system) and obtained positive results. 
...
concept.\footnote{We plan to give a demo if our contribution would be accepted.}  
\end{quote} 
\end{abstract}
...
% I think we would need to get references on robot dialog systems in our next version if we're accepted.
...
\begin{figure*}[!t] 
\centering 
\includegraphics[width=.7\textwidth]{figures/system} 
...
%as a platform provides the additional advantages (e.g. in addition to assessing, say a lack of exercise, it can also demonstrate physical exercises).
