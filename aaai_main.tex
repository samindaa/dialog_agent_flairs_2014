...
\maketitle
\begin{abstract}
We have combined a spoken dialog system that we have developed to deliver brief health 
interventions with a 3D anthropomorphic speech-enabled interface to a fully autonomous humanoid 
robot (NAO). The dialog system is based on Markov decision processes (MDPs). We have used model 
free off-policy algorithms in reinforcement learning (RL) with a dataset collected from real user 
interactions to obtain a (near-) optimal policy. The system begins to learn optimal dialog 
strategies for initiative selection and for the type of confirmations that it uses during the
...
\begin{figure}[!t] 
\centering 
\includegraphics[width=.47\textwidth]{figures/dialog_manager_v2} 
...
state is associated with 2 actions  based on initiatives and  confirmation types. The reward 
...
\begin{figure}[!t] 
\centering 
\includegraphics[width=.47\textwidth]{figures/system} 
